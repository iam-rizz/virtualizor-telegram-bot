...
class Database:
    ...
    async def add_api(self, name: str, api_url: str, api_key: str, api_pass: str) -> bool:
    ...
    async def get_api(self, name: str) -> Optional[Dict[str, Any]]:
        async with aiosqlite.connect(self.db_path) as conn:
            ...
            cursor = await conn.execute("SELECT * FROM api_configs WHERE name = ?", (name,))
    ...
    async def get_default_api(self) -> Optional[Dict[str, Any]]:
        async with aiosqlite.connect(self.db_path) as conn:
            conn.row_factory = aiosqlite.Row
            cursor = await conn.execute("SELECT * FROM api_configs WHERE is_default = 1")
            row = await cursor.fetchone()
            return dict(row) if row else None
    ...
    async def delete_api(self, name: str) -> bool:
        async with aiosqlite.connect(self.db_path) as conn:
            cursor = await conn.execute("DELETE FROM api_configs WHERE name = ?", (name,))
            await conn.commit()
    ...
    async def set_default(self, name: str) -> bool:
        async with aiosqlite.connect(self.db_path) as conn:
            ...
            cursor = await conn.execute("UPDATE api_configs SET is_default = 1 WHERE name = ?", (name,))
            await conn.commit()
            return cursor.rowcount > 0

    async def api_exists(self, name: str) -> bool:
        async with aiosqlite.connect(self.db_path) as conn:
            cursor = await conn.execute("SELECT 1 FROM api_configs WHERE name = ?", (name,))
